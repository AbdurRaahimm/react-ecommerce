...
const Hero = () => {
  ...
  const prevSlider = () => setCurrentSlider((currentSlider) => currentSlider === 0 ? products.length - 1 : currentSlider - 1);
  const nextSlider = () => setCurrentSlider((currentSlider) => currentSlider === products.length - 1 ? 0 : currentSlider + 1);
  ...
  return (
    <div className="w-full h-60 sm:h-96 md:h-[540px] flex flex-col xl:flex-row items-center justify-center gap-5 lg:gap-10 relative bg-cover before:absolute before:bg-black/50 before:inset-0 transform duration-1000 ease-linear z-50 overflow-hidden"
      style={{ backgroundImage: `url(${currentSlider === 0 ? products[products.length - 1].image : products[currentSlider - 1].image})` }}>
      {/* arrow */}
